...
class SamplingParams(BaseModel):
    ...
    repetition_penalty: float = Field(default=1.0, gt=0.0, le=2.0, description="重复惩罚")
    stop: list[str] | None = Field(default=None, description="停止词列表")
...
class InferenceRequest(BaseModel):
    ...
    session_id: str | None = Field(default=None, description="会话ID，用于上下文")
    priority: int = Field(default=5, ge=0, le=10, description="请求优先级")
    tags: dict[str, str] | None = Field(default=None, description="标签，用于资源隔离")
    request_id: str | None = Field(default=None, description="请求ID，不提供则自动生成")
...
class DeployRequest(BaseModel):
    ...
    max_model_length: int | None = Field(default=None, ge=1, le=32768, description="最大模型长度")
    ...
    target_gpu_utilization: float = Field(default=0.7, ge=0.1, le=1.0, description="目标GPU利用率")
    quantization: str | None = Field(default=None, description="量化方式: awq, gptq, gguf")
...
class ModelFilterRequest(BaseModel):
    ...
    backend: str | None = Field(default=None, description="后端过滤")
    labels: dict[str, str] | None = Field(default=None, description="标签过滤")
...
class BenchmarkRequest(BaseModel):
    ...
    model: str = Field(..., description="模型名称")
    test_set: str = Field(default="mmlu", description="测试集: mmlu, humaneval, math, custom")
