...
class BenchmarkResult:
    ...
    def summary(self) -> dict:
        return {
            ...
            "avg_gpu_mem_util": (
                np.mean(self.gpu_mem_util_samples) if self.gpu_mem_util_samples else 0
            ),
            ...
            "throughput_tok_s": (
                np.mean(self.throughput_tokens_per_sec) if self.throughput_tokens_per_sec else 0
            ),
        }
...
async def run_scenario(
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        for r in range(rounds):
            ...
            tasks = [
                send_request(client, PROMPTS[i % len(PROMPTS)], max_tokens)
        ...
        print(
            f"    Latency:         avg={s['avg_latency_ms']:.0f}ms  p50={s['p50_latency_ms']:.0f}ms  p95={s['p95_latency_ms']:.0f}ms"
        )
...
def generate_charts(results: list[BenchmarkResult], output_path: str):
    ...
    ax.errorbar(
        x,
        avg_u,
        yerr=[max(0, m - a) for m, a in zip(max_u, avg_u)],
        fmt="none",
        color=C["text"],
        capsize=3,
        capthick=0.8,
    ...
    row_data = [
        ...
        [
            "GPU Mem BW Util",
            f"{summaries[0]['avg_gpu_mem_util']:.1f}%",
            f"{summaries[1]['avg_gpu_mem_util']:.1f}%",
            f"{summaries[2]['avg_gpu_mem_util']:.1f}%",
        ],
        [
            "P50 Latency",
            f"{summaries[0]['p50_latency_ms']:.0f} ms",
    ...
    print("-" * 70)
    print(
        f"{'GPU计算利用率':<20} {summaries[0]['avg_gpu_util']:.1f}%{'':<9} {summaries[1]['avg_gpu_util']:.1f}%{'':<9} {summaries[2]['avg_gpu_util']:.1f}%"
    )
    print(
        f"{'GPU显存带宽':<20} {summaries[0]['avg_gpu_mem_util']:.1f}%{'':<9} {summaries[1]['avg_gpu_mem_util']:.1f}%{'':<9} {summaries[2]['avg_gpu_mem_util']:.1f}%"
    )
    print(
        f"{'P50延迟':<20} {summaries[0]['p50_latency_ms']:.0f}ms{'':<9} {summaries[1]['p50_latency_ms']:.0f}ms{'':<9} {summaries[2]['p50_latency_ms']:.0f}ms"
    )
    print(
        f"{'P95延迟':<20} {summaries[0]['p95_latency_ms']:.0f}ms{'':<9} {summaries[1]['p95_latency_ms']:.0f}ms{'':<9} {summaries[2]['p95_latency_ms']:.0f}ms"
    )
    print(
        f"{'吞吐量(tokens/s)':<20} {summaries[0]['throughput_tok_s']:.0f}{'':<14} {summaries[1]['throughput_tok_s']:.0f}{'':<14} {summaries[2]['throughput_tok_s']:.0f}"
    )
    print(
        f"{'成功率':<20} {summaries[0]['success']/max(summaries[0]['total_requests'],1)*100:.0f}%{'':<9} {summaries[1]['success']/max(summaries[1]['total_requests'],1)*100:.0f}%{'':<9} {summaries[2]['success']/max(summaries[2]['total_requests'],1)*100:.0f}%"
    )
    ...
    if summaries[0]["throughput_tok_s"] > 0:
        ...
        print(
            f"  📈 吞吐量提升: {tp_improvement:.1f}x ({summaries[0]['throughput_tok_s']:.0f} → {summaries[2]['throughput_tok_s']:.0f} tok/s)"
        )
